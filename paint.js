function Paint(p) {
    ...
    var maxSpeed = 3.0; //3.0
    var perception = 0.9; //5\\4.6
    var bound = 70; //60
    var boundForceFactor = 0.01; //0.16
    var noiseScale = 2.0; //100.0
    var noiseInfluence = 1 / 40.0; // 1 / 20.0

    var dropRate = 0.001; //0.004
    var dropRange = 4; //40
    var dropAlpha = 150; //150
    var drawAlpha = 50; //50
    var drawColor = color(0, 0, 0, drawAlpha);
    var drawWeight = 0.9; //1.0
    var count = 0;
    var maxCount = 35; //100

    this.update = function(noiseScale) {
    ...
    this.show = function() {
        ...
        if (force.mag() > 0.1 && random(1) < dropRate) {
            ...
            stroke(drawColor);
            var boldWeight = drawWeight+random(4);
