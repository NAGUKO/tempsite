...
function preload() {
    imgs[0] = loadImage("bbth3.png");
    imgs[1] = loadImage("bbth3.png");
    //imgs[2] = loadImage("logo.png");
}

function setup() {
    //if(windowWidth < 600) //600
    //	createCanvas(windowHeight, windowWidth); //createCanvas(windowWidth, windowWidth);
    //else
    createCanvas (window.innerWidth,window.innerWidth / 2);//600,600
...
function draw() {
    ...
    if (!isStop) {
        for (var i = 0 ; i < subStep ; i++) {
            paint.update(parameters.noiseScale);
            paint.show();
            z+= 0.01;
        }
    }
